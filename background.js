...
async function handleScanLease(message) {
  ...
  const stored = await chrome.storage.local.get(SCAN_LEASE_KEY);
  ...
  if (message.type === "ACQUIRE_SCAN_LEASE") {
    ...
    await chrome.storage.local.set({ [SCAN_LEASE_KEY]: nextLease });
  ...
  if (message.type === "RELEASE_SCAN_LEASE") {
    if (lease?.owner === owner) await chrome.storage.local.remove(SCAN_LEASE_KEY);
